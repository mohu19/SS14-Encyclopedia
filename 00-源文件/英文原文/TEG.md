...
> 条目ID: `TEG` ｜ 来源层: official ｜ 分类: engineering
...
# Thermo-electric Engine (TEG)
  ...
  The gas then gets output on the other end of the circulator. The generator produces the actual power and outputs it over an <span style="color:orange">HV wire</span>.

  Note that the circulators are <span style="color:#a4885c">directional</span>: they will only let gas through one way.
  ...
  The gasses in the two "loops" are never mixed, <span style="color:#a4885c">only energy is exchanged between them</span>.
  The hot side will cool down, the cold side will heat up.

  ## The Pipes

  There are 2 major [textlink="pipenets" link="PipeNetworks"] to worry about here: <span style="color:red">The Hot Loop</span> (where gas will be burnt for heat), and <span style="color:cyan">The Cold Loop</span> (where circulated, heated waste gas will either be removed into space or cooled back down).
  Make sure that <span style="color:#a4885c">**both [textlink="pipenets" link="PipeNetworks"] do NOT mix**</span>, as only heat should be transferred between the two through the TEG.
  ...
  As stated above, there are many different layouts one can follow to heat up (or cool down) gasses; this part of the guide will cover some common methods one will often see for the hot loop, involving <span style="color:red">the Burn Chamber</span>.
  ...
  There is a notable difference between the [textlink="passive vent" link="PassiveVent"] and the [textlink="air injector" link="AirInjector"]; the [textlink="air injector" link="AirInjector"] can only keep injecting air up to <span style="color:#a4885c">9MPa</span>, which can be reached very easily with a good burn.
  ...
  Note that this is just one of many ways you can setup the hot loop; <span style="color:#a4885c">feel free to mix and match setups as needed!</span>
  ...
  There are 3 main methods you will see used for the Cold Loop: <span style="color:#a4885c">The Water Cooler</span> (see: Liltenhead's video on the TEG), <span style="color:cyan">the Coolant Array</span> and <span style="color:#a4885c">the Freezer Loop</span>.
  ...
  Being of moderate complexity and having no losses of any resource, this <span style="color:#a4885c">should</span> be the main method of cooling down the TEG.
  ...
  To use heat-exchangers properly, they must be setup in <span style="color:#a4885c">parallel</span>, not in series (like what you see on most stations).
  ...
  The flow-rate of the volumetric pump should be set using the following formula:
    ...
    <span style="color:cyan">( 200 / number of heat-exchangers )</span> L/s.
  ...
  - Radiator: Basically, just a bunch of radiators in space. Not much to say, besides the fact that it cools down the gas inside it. Make sure the radiators are <span style="color:#a4885c">placed on lattice, not plating</span>! Otherwise, the heat-exchange efficiency will be greatly reduced, as the radiators aren't directly exposed to space below them.
  ...
  <span style="color:#a4885c">Experiment!</span>
  ...
  **It's like a giant puzzle, so go out and solve it!**
