...
> 条目ID: `InspectingPower` ｜ 来源层: official ｜ 分类: engineering
...
﻿
  ...
  The Power Monitoring Computer is a device that can be used to inspect all the power networks on the station.
    ...
    [[实体:ComputerPowerMonitoring]]
  

  The Power Monitoring Computer has two major sections, a station map and the power statistics.

  The station map shows:
  - The station layout, with <span style="color:green">low</span>, <span style="color:yellow">medium</span>, and <span style="color:orange">high</span> [textlink="voltage networks" link="VoltageNetworks"] highlighted.
  ...
  A <span style="color:green">low voltage</span> cable (which likely only services a few rooms) will not show the same statistics as a <span style="color:orange">high voltage</span> cable (which services the entire station).
